...
export type CommandNames = { command: string; subCommandGroup?: string; subCommand?: string };
export type OptionAndValue = ApplicationCommandOptionData & { value: any };

const interactionsFolderPath = resolve(__dirname, "../interactions/Commands/ChatInputCommands");

(async () => {
	for await (const file of readdirSync(interactionsFolderPath).filter((file) => file.endsWith(".js"))) {
		console.log(`Chargement de la commande ${interactionsFolderPath.split(/\\|\//g).at(-1)}/${file}`);
		let command = await import(resolve(interactionsFolderPath, file)).then((m) => m.default as ChatInputApplicationCommandData);
		if (command.name === "dev") continue;
		localCommands.set(command.name, command);
	}
	console.debug(
		`The following commands have been loaded by discordMock: \n${Array.from(localCommands.keys())
			.map((n) => `- ${n}`)
			.join("\n")}`
	);
})();
...
export async function getNewInteraction(interaction: ChatInputCommandInteraction, commandNames: CommandNames, options: OptionAndValue[] = []) {
...
export async function getCommandOptions(i: ChatInputCommandInteraction, commandInput: CommandNames): Promise<Map<string, OptionAndValue>> {
	let { command, subCommandGroup, subCommand } = commandInput;
	if (!localCommands.has(command)) throw new Error("Cette commande n'existe pas");
	let localCommand: ChatInputApplicationCommandData | ApplicationCommandSubGroupData = localCommands.get(command)!;
	...
	let executableCommand: ChatInputApplicationCommandData | ApplicationCommandSubCommandData | ApplicationCommandSubGroupData;

	if (subCommandGroup) {
		subCommandGroupData = localCommand.options?.find(
			(option) => option.name === subCommandGroup && option.type === ApplicationCommandOptionType.SubcommandGroup
		) as ApplicationCommandSubGroupData;
		if (!subCommandGroupData) throw new Error(`SubGroup ${subCommandGroup} doesn't exists on command ${command}`);
	}

	subCommandData = localCommand.options?.find(
		(option) => option.name === subCommand && option.type === ApplicationCommandOptionType.Subcommand
	) as ApplicationCommandSubCommandData;
