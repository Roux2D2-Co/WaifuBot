	...
	private static handler = (interaction: BaseInteraction, ...args: any[]): void => {
		if (InteractionHandler.paused) return;
			...
			if (interaction.isChatInputCommand()) {
				let localInteraction = client.localCommands.get(ApplicationCommandType.ChatInput)?.get(interaction.commandName) ?? null;
				...
				if (!!localInteraction) {
					localInteraction.execute(interaction as Interaction<CacheType>, regexResult);
				} else {
					console.error(`No local interaction found for ${interaction.customId}`);
				}
			} else if (interaction.isAutocomplete()) {
				let localInteraction =
					(client.localCommands.get(ApplicationCommandType.ChatInput)?.get(interaction.commandName) as ChatInputApplicationCommandData) ??
					null;
				localInteraction?.onAutocomplete?.(interaction as Interaction<CacheType>);
			}
		}
	};
